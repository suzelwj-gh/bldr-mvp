...
const NEW_EMAIL = 'conroy@cpjenterprise.com';
const NEW_NAME = 'Conroy Parchment';
const NEW_ROLE = 'superintendent';

async function run() {
  const source = await pool.query(
    `SELECT password_hash, project, pm_email FROM users WHERE email = $1`,
    [SOURCE_EMAIL]
  );
  ...
  const { password_hash, project, pm_email } = source.rows[0];
  ...
  const result = await pool.query(
    ...
    [NEW_NAME, NEW_EMAIL, password_hash, NEW_ROLE, project || 'ARI Demo', pm_email || SOURCE_EMAIL]
  );
...
run().catch((err) => {
  console.error('add-conroy-superintendent failed:', err);
  process.exit(1);
});
